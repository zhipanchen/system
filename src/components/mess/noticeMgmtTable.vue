				...
				<tbody>
					<tr v-for="(notice, index) in announcementList" :key="notice">
						<td :value="notice.announcementId">
							<a id="signIn" :href="'#/notify?'+notice.announcementId">{{notice.announcementName}}</a>
						</td>
						...
						<td>
							...
							<Modal v-model="modal1" id="modalBody" :styles="{top:'10rem'}">
								...
								<div slot="footer" style="text-align:center;">
									<Button id="modalBtn" @click="deleteOk(index)">确定</Button>
...
export default {
    ...
    methods: {
    	...
    	// 点击添加通知，跳转到发布的页面
    	addClick: function () {
    		window.location.href = "#/eduAdmin/notice/noticeSetting"
    	}
    }
}
</script>
